...
class App extends React.Component {
  ...
  componentDidMount(){
    ...
    fetch(url)
      ...
      .then(data => {
        this.setState({quotes: JSON.stringify(data)});
        let rawQuotes = data.quotes;
        let oneQuote = rawQuotes[Math.floor(Math.random()*rawQuotes.length)];
        this.setState({currentQuote: oneQuote.quote })
        this.setState({currentAuthor: oneQuote.author })
      })
  ...
  refresh(){
    fetch(url)
      ...
      if (newQuote.quote == this.state.currentQuote || this.state.sameQuoteTest == "1"){
        ...
        this.setState({sameQuoteTest : "0"})
        this.refresh()
      }else {
        this.setState({currentQuote : newQuote.quote})
        this.setState({currentAuthor : newQuote.author})
      }
      })
   ...
   render(){
    return(
      <div className = "Wrapper" id="quote-box">
        ...
        <div id = "author-box">
        <h3 id = "author"> - {this.state.currentAuthor}</h3>
        </div>
        </div>
        <div id = "buttons">
        <button onClick = {this.refresh} id = "new-quote">New Quote</button>
        {/*<button onClick = {this.test}>TEST : {this.state.sameQuoteTest}</button>*/}
        
        </div>
        <div id="tweet-quote-padding">
        <a id="tweet-quote" href={"https://twitter.com/intent/tweet?hashtags=quotes&related=freecodecamp&text="+ '"' + this.state.currentQuote + '" ' + " -" + this.state.currentAuthor}>Tweet <i class="fa fa-twitter" ></i></a>
